...
DB_CONFIG = {
    ...
    "database": os.getenv("DB_NAME", "your_erp_db"),
...
# API Auto Parts Catalog (RapidAPI / Apify)
API_CONFIG = {
...
DATA_SOURCE = "auto_parts_catalog"  # ou "tecdoc"
BATCH_SIZE = 100
...
class AutoPartsCatalogAPI:
    """Client pour Auto Parts Catalog API (RapidAPI)"""
    ...
    def get_manufacturers(self) -> List[Dict]:
        """Récupère la liste des marques (Bosch, Valeo, etc.)"""
        return self._get("/manufacturers")

    def get_articles_by_vehicle(self, car_id: int, page: int = 1) -> List[Dict]:
    ...
    def get_article_details(self, article_id: str) -> Dict:
        """Détails complets d'une pièce (images, specs, compatibilité)"""
        return self._get(f"/articles/{article_id}")

    def get_article_specifications(self, article_id: str) -> List[Dict]:
        """
        Récupère les spécifications techniques d'une pièce.
        Retourne: [{"name": "Height", "value": "12.5", "unit": "cm"}, ...]
        """
        try:
            return self._get(f"/articles/{article_id}/specifications")
    ...
    def search_by_oem(self, oem_number: str) -> List[Dict]:
        ...
        return self._get("/search/oem", params={"number": oem_number})
