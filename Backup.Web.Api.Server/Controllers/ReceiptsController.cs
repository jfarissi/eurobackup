...
namespace Backup.Web.Api.Server.Controllers
{
    ...
    public class ReceiptsController : RESTFulController
    {
        ...
        public async Task<IActionResult> Comptabiliser([FromBody] ComptabiliserRequest request)
        {
            ...
            {
                var alreadyStocked = this.storage.SelectStockUpdatesByDeliveryId(request.DocumentId).Any();
                if (alreadyStocked)
                {
                    result.StockAlreadyApplied = true;
                    result.Warnings.Add($"Stock déjà alimenté pour le BL #{request.DocumentId} (pas de double entrée).");
                }
                else
                {
                    var stockChanges = created.Lines
                    ...
                    {
                        await this.storage.UpsertStockBatchAsync(stockChanges, request.DocumentId, invoiceId: null);
                        ...
                        foreach (var change in stockChanges)
                        {
                            await this.storage.InsertStockMovementAsync(new StockMovement
                            {
                                ...
                                ReferenceDocument = $"REC:{created.ReceiptNumber}|BL:{request.DocumentId}",
                                CompanyId = this.companyContext.GetCurrentCompanyId(),
                                CreatedBy = createdBy,
                                CreatedAt = DateTime.UtcNow
                            });
            ...
            // Optionnel : mettre à jour qté reçue sur la CFA liée
            if (request.PurchaseOrderId.HasValue)
            {
                var po = await this.storage.SelectPurchaseOrderByIdAsync(request.PurchaseOrderId.Value);
                if (po != null)
                {
                    var receivedByKey = created.Lines
                        .GroupBy(l => ProductKeyHelper.Normalize(l.ProductKey), StringComparer.OrdinalIgnoreCase)
