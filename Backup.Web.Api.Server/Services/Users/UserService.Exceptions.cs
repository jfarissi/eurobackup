...
namespace Backup.Web.Api.Server.Services.Users
{
    public partial class UserService
    {
        ...
        private delegate ValueTask<string> ReturninglisteRoleFunction();
        private delegate ValueTask<AuthenticateResponse> ReturningAuthenticateResponseFunction();
        private delegate IQueryable<User> ReturningQueryableUserFunction();

        private async ValueTask<AuthenticateResponse> TryCatch(ReturningAuthenticateResponseFunction returningUserFunction)
        {
            try
            {
                return await returningUserFunction();
            }
            catch (NullRoleException nullUserException)
            {
                throw CreateAndLogValidationException(nullUserException);
            }
            catch (InvalidRoleException invalidUserException)
            {
                throw CreateAndLogValidationException(invalidUserException);
            }
            catch (NotFoundRoleException nullUserException)
            {
                throw CreateAndLogValidationException(nullUserException);
            }
        ...
        private async ValueTask<string> TryCatch(ReturninglisteRoleFunction returningUserFunction)
        {
            try
            {
                return await returningUserFunction();
